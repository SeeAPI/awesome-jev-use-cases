# What the evidence establishes / 证据能说明什么
...
中文：来源核查、文档核查、作者演示、SeeAPI 实际运行与独立基准是不同证据类型，不是一条通用质量排名。推荐先看任务匹配，再比较相关证据；代码审阅、作者自测与独立运行必须分开。结构化迁移不会自动将案例升级为“实测”。未知字段应明确说明，不能补造准确率、延迟或费用。
